...
from cubed.runtime.executors.lithops_async import map_unordered
from cubed.runtime.executors.lithops_async_wrapper import AsyncFunctionExecutorWrapper


async def run_test(function, input, retries, timeout=10, use_backups=False):
    outputs = set()
    with AsyncFunctionExecutorWrapper(
        RetryingFunctionExecutor(LocalhostExecutor())
    ) as executor:
